...
from .source_data import repeat_interrupt, repeat_whisper
...
async def repeat_judger(event : Event) -> None:
    ...
    if group_id not in E_messsage: 
        E_messsage[group_id] = {'text' : plain_text, 'times' : 0}
    
    # 若信息变换, 重置消息和中对应的内容
    # 另外, 若该群不在interrupt集合中并在whisper集合中, 发送wisper内容
    if E_messsage[group_id]['text'] != plain_text:
        if group_id not in  Repeat_judger_args_dic['interrupt_group'] and group_id in Repeat_judger_args_dic['whisper_group']:
            if E_messsage[group_id]['times'] >= Repeat_judger_args_dic['threshold']:
                await E_repeat_judger.send(await repeat_whisper(user_id, E_messsage[group_id]['times']))
        E_messsage[group_id] = {'text' : plain_text, 'times' : 0}
    ...
    if group_id in Repeat_judger_args_dic['interrupt_group']:
        if E_messsage[group_id]['times'] >= Repeat_judger_args_dic['threshold']:
            await E_repeat_judger.send(await repeat_interrupt(user_id))
